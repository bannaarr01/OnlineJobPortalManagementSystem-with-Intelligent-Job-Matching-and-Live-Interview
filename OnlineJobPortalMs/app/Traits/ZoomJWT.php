...
trait ZoomJWT
{
    ...
    public function toZoomTimeFormat(string $dateTime, string $time)
    {
        try {
            $datenTime = $dateTime . $time;
            $date = new \DateTime($datenTime);
    ...
    public function toUnixTimeStamps(string $dateTime, string $time, string $timezone)
    {
        try {
            $date = new \DateTime($dateTime . 'T' . $time, new \DateTimeZone($timezone));
            return $date->getTimestamp();
        } catch (\Exception $e) {
            Log::error('ZoomJWT->toUnixTimeStamp : ' . $e->getMessage());
            return '';
